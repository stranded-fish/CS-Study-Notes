...
* `path`：当前路径
  ...
  * 情景 2 - 如：求解 N 皇后不同的解决方案的数量。由于此时仅需要求出答案数量，`path` 不作为最终的结果要求且不用于回溯。故此时不需要显性的 `path` 记录（通常改设一个简单变量用于判断是否产生题目所需解）。
* `selectList`：选择列表
...
* **回溯算法需要在递归之后，重置状态，而 DFS 通常不需要**，这是由于 DFS 通常不需要维护大量结果 `path` 或者 约束条件 `constraint`，故可以采用内存消耗更多的复制传递，而不是回溯的引用传递。在部分复制传递消耗较小的情况下，也可以将回溯算法中的引用传递改为复制传递，此时可以省略递归后的回溯步骤，程序结构将与 DFS 更加相似。
